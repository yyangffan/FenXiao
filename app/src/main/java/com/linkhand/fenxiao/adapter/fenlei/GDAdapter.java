...
public class GDAdapter  extends BaseAdapter {
    ...
    List<RightClassFeng.InfoBean.CateBean> results;

    public GDAdapter(Context context, List<RightClassFeng.InfoBean.CateBean> results) {
...
//            Category c = (Category) getItem(position);
        ...
        if (this.results != null) {
            String cate_id = results.get(position).getCate_id();//商品id
            String thrmb = results.get(position).getCate_img();//商品图片

            convertView.setTag(holder);
            holder.tv.setText(results.get(position).getCate_name());
